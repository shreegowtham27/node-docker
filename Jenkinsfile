node {
    ...
    stage('tag image') {
        sh "docker tag docker-node:latest 228645407764.dkr.ecr.us-east-1.amazonaws.com/docker-node:${BUILD_NUMBER}"
    }
    stage('Push image') {
        docker.withRegistry('https://228645407764.dkr.ecr.us-east-1.amazonaws.com', 'ecr:us-east-1:AWS') {
            sh "docker push 228645407764.dkr.ecr.us-east-1.amazonaws.com/docker-node:${BUILD_NUMBER}"
        }
    }
}
